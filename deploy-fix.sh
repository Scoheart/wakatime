...
echo -e "${BLUE}===== WakaTime Dashboard 部署脚本（跳过类型检查）=====${NC}"
echo -e "${BLUE}将部署到服务器: ${SERVER_IP}:${PORT}${NC}"

# 1. 构建项目 - 跳过TypeScript类型检查
echo -e "${BLUE}[1/5] 准备构建项目...${NC}"

# 修改vite.config.ts以跳过类型检查
...
echo -e "${BLUE}[2/5] 开始构建项目 (跳过类型检查)...${NC}"
...
# 5. 在服务器上安装和配置 Nginx
echo -e "${BLUE}[5/5] 检查并安装 Nginx...${NC}"
